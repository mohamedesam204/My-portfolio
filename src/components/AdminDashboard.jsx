...
import { FaEdit, FaTrash, FaPlus, FaSave, FaTimes, FaCog, FaChartBar, FaTasks } from "react-icons/fa";
...
export default function AdminDashboard({ isAdmin, onLogout }) {
  ...
  e.preventDefault();
  const updates = { name: portfolioName, subtitle: portfolioSubtitle, description: portfolioDescription };
  if (cvFile) {
    const reader = new FileReader();
    reader.onload = () => {
      updates.cvUrl = reader.result;
...
const handleCVChange = (e) => {
  ...
  <div className="space-y-6">
    ...
    <form onSubmit={handlePortfolioSubmit} className="grid gap-4 max-w-lg">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">CV File</label>
        <input type="file" accept=".pdf" onChange={handleCVChange} className="mt-1" />
      </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-400 text-white text-xs font-bold transition-all shadow-md"
              >
            ...
            <div className="border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-450 border-b border-slate-200 dark:border-white/5 font-semibold text-xs uppercase tracking-wider">
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-950 border border-slate-305 dark:border-slate-800 focus:border-sky-500 outline-none text-sm transition-colors text-slate-800 dark:text-white"
                />
              </div>
